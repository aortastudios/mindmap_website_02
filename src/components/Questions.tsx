...
const Questions = ({ question, answer }: QuestionProp) => {
  ...
  return (
    ...
    >
      ...
      <div
        onClick={handleClick}
        className="w-full flex justify-between px-5 lg:px-10 items-center h-20 lg:h-23 relative"
      >
        <span
        ...
        {isOpen && (
          <div className="h-5 w-0.5 lg:w-1 bg-primary-100 absolute top-7 lg:top-9 left-0 "></div>
        )}
      ...
      {/* Answer */}
      <div
        className={`overflow-hidden transition-all duration-700 ease-in-out  flex gap-2  ${
